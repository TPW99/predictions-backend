...
const runScoringProcess = async () => {
    ...
    try {
        ...
        // --- THIS IS THE FIX (TEMPORARY) ---
        // We are temporarily fetching ALL finished fixtures to force a full recalculation
        const fixturesToScore = await Fixture.find({
            kickoffTime: { $lt: new Date() },
            // 'actualScore.home': null  <-- TEMPORARILY REMOVED
        });
...
app.post('/api/admin/score-gameweek', authenticateToken, authorizeAdmin, async (req, res) => {
...
app.post('/api/admin/update-score', authenticateToken, authorizeAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/update-fixture', authenticateToken, authorizeAdmin, async (req, res) => {
    try {
